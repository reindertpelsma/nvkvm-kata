...
| Does the whole thing work? | **MEASURED: yes.** CUDA in a container in a Kata VM through nvkvm, 2026-08-27, RTX 4070 Ti SUPER; two sandboxes sharing one GPU; ~3.8 s sandbox start. One upstream line changed. But the container's device cgroup is **not enforced in the guest** — `cgroups-rs` has no v2 devices subsystem, the twin of the host-side bug in [01 §1.6](01-vmm-confinement.md) — so the boundary [03](03-guest-side-devices.md) relies on is absent. | [07](07-end-to-end.md) |
