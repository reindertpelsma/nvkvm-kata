...
> **Note added when this branch was merged.** Both fixes landed on `main` in a
> *different* form from the one this branch carried, because `main` had moved on
> (the `gpu-libs` work) and `pc-install`'s installer was a snapshot of an older
> working tree. The driver-version rule now lives once in
> `scripts/lib/driver-version.sh`, sourced by the installer and mirrored by
> `gpu-cdi.py`, and it also handles a case this host did show but this branch did
> not fix: a **two-component** version (`595.84`), which the old rule answered
> with the GCC version from the file's second line. `--qemu` is resolved at
> argument-parse time. Both are regression-tested — see
> `scripts/lib/gpu-cdi.py self-test` and evidence/11. Only this document was
> taken from `pc-install`; every other file on that branch was already present
> on `main` in a newer form.
