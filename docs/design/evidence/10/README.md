...
| [`readme-commands.log`](readme-commands.log) | **every command in the README, executed verbatim** — the installer, the `docker run` one-liner, the compose file, the Docker-less `ctr` form, and `--gpus 1` / `"device=0"` / `"device=<uuid>"` |
